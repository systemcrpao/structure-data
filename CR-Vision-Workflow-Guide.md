...
- **สิทธิ์การเข้าถึง:** เข้าถึงได้ _ทุกฟังก์ชัน_ ของระบบและสามารถ Bypass Rule ของ Role อื่นได้ทั้งหมด
...
### 3.2 👨‍🔧 User (ผู้บันทึกข้อมูล / ผู้ปฏิบัติงาน)
...
- **UI หลัก (`treasury.html`):**
  - แสดงสถานะปัจจุบัน (badge สีตามสถานะ) และ Dropdown เปลี่ยนสถานะใหม่ แยก column ชัดเจน
  - **Dirty Tracking:** แถวที่มีการเปลี่ยนสถานะจะถูก Highlight ด้วย border ซ้ายสีน้ำเงินม่วง เพื่อแยกรายการที่ยังไม่บันทึก
  - **บันทึกทีละแถว:** กด "บันทึก" ในแถวนั้น
  - **บันทึกทั้งหมด:** ปุ่ม "บันทึกทั้งหมด" (พร้อม badge นับจำนวนที่เปลี่ยน) สำหรับบันทึก Batch พร้อมกันทุกรายการที่แก้ไข
  - **Pagination:** รองรับ page-size selector (10 / 20 / 50 / 100 / ทั้งหมด) พร้อม pagination buttons แบบเดียวกับ `technician.html`
...
- **ปุ่มมุมล่างซ้าย (Sidebar Footer):** แสดง "เข้าสู่ระบบ" สำหรับผู้ที่ยังไม่ Login → นำทางไปหน้า `login.html`
...
- **Mobile: ปุ่ม Login มุมซ้ายบน (top-left):** แสดงเฉพาะ mobile (`xl:hidden`) เป็น icon `log-in` พร้อม label "เข้าสู่ระบบ" เมื่อยังไม่ Login และเปลี่ยนเป็น icon `layout-dashboard` + ชื่อผู้ใช้เมื่อ Login แล้ว การกดจะเรียกฟังก์ชัน `goToLogin()` เช่นเดียวกัน
